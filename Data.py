...
class KmeansCluster(object):
    ...
    def get_cluster(self, num_cluster, max_iter=10):
        ...
        for it in range(max_iter):
            ...
            cluster = self._cluster_pts(centers)
    ...
    def _cluster_pts(self, centers):
        """Lloyd’s algorithm for performing k-means clustering"""
        # reference:
        # https://datasciencelab.wordpress.com/2013/12/12/clustering-with-k-means-in-python/
        cluster = {}
        for v in self._data:
            bestcenterkey = min([(i[0], np.linalg.norm(v-centers[i[0]]))
                         for i in enumerate(centers)], key=lambda t:t[1])[0]
            try:
                cluster[bestcenterkey].append(v)
            except KeyError:
                cluster[bestcenterkey] = [v]
        return cluster
...
if __name__ == "__main__":
    ...
    err,rst = my_cluster.get_cluster(num_cluster,max_iter=10)
